...
Just as when we constructed the [name ring](http://blog.mathzorro.com/2017/03/Creating-a-Name-Ring.html), we need to finish the cylinders with spheres at each endpoint.  We need spheres of the same radius as the radius of the adjacent cylinders.
...
Now let's print out the model!  (Note that I provide much more detail on the prototyping and printing processes at the bottom of the [name ring design post](http://blog.mathzorro.com/2017/03/Creating-a-Name-Ring.html).)
...
The standard Cura software that comes with the Lulzbot was not able to correctly process my file—it either completely fills in the bottom half of the model or does not connect the edges together.  So I needed to do some post-processing of the STL file.  I uploaded it to [Netfabb's online STL repair service](https://service.netfabb.com/login.php) and the Lulzbot was able to print the STL output from there with no issues.
:::
